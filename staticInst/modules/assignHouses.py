...
def assign_individuals_to_houses(targetPopulation, wards, totalHousehold, ageDistribution=[0.073, 0.078, 0.083, 0.086, 0.102, 0.11, 0.098, 0.081, 0.071, 0.059, 0.049, 0.037, 0.03, 0.021, 0.01, 0.007, 0.005], householdDistribution=[0.0417, 0.1308, 0.2228, 0.3077, 0.1530, 0.1439]):
    ...
    if len(unassigned_households_ids)>0 and len(unassigned_households_ids) <= len(unassigned_individuals_ids):
        ...
        for j in range(0,len(house_indices)):
            ...
            households.at[unassigned_households_ids[house_indices[j]],'flag'] = 1
    elif len(unassigned_households_ids)>0 and len(unassigned_households_ids) > len(unassigned_individuals_ids):
