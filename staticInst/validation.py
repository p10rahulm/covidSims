...
# age distribution
    ...
    individuals = pd.read_json('./data/bangalore-schoolSize-1700/individuals.json')
    ...
    workplace_distances = []    
    wp = pd.read_json('./data/bangalore-schoolSize-1700/workplaces.json')
    ...
    a = plt.hist(workplace_distances,bins=100,density=True,histtype='step')
    plt.close()
    plt.plot((a[1][0:100]),np.log((a[0][0:100])))
    plt.xlabel('Workplace distance (km)')
    plt.ylabel('log-Density')
