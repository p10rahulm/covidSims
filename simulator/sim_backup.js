...
//global variables.
NUM_PEOPLE = 1000000; // Number of people. Will change once file is read.
...
function compliance(){
    const val = (Math.random() < COMPLIANCE_PROBABILITY) ? 1 : 0;
    return val;
}
...
function compute_prob_infection_given_community(infection_probability,set_uniform){
    ...
    const prob_infec_given_community = [];
    const communities_population_json = JSON.parse(loadJSON_001('input_files/fractionPopulation.json'));
    if(!set_uniform){
        var communities_frac_quarantined_json = JSON.parse(loadJSON_001('input_files/quarantinedPopulation.json'));
    }

    const num_communities = communities_population_json.length;
    for (let w = 0; w < num_communities; w++){
...
function init_nodes() {
    ...
    const individuals_json = JSON.parse(loadJSON_001('input_files/individuals.json'));
    const workplace_json = JSON.parse(loadJSON_001('input_files/workplaces.json'));
...
function load_infection_seed_json(seed_scaling_factor){
    const seed_array = [];
    const infection_seeds_json = JSON.parse(loadJSON_001('input_files/infection_seeds.json'));
    const infection_seeds_json_array = Object.values(infection_seeds_json['seed_fit']);
    const num_seed_days = infection_seeds_json_array.length;
    for (let count = 0; count < num_seed_days*SIM_STEPS_PER_DAY; count++){
        const mean = (infection_seeds_json_array[Math.floor(count / SIM_STEPS_PER_DAY)] / SIM_STEPS_PER_DAY) * seed_scaling_factor;
...
function infection_seeding(nodes,seed_count,curr_time){
    const num_individuals_to_seed = seed_count;
    let num_seeded = 0;
...
function infection_seeding_exp_rate(nodes,curr_time){
    if(curr_time >= SEEDING_START_DATE*SIM_STEPS_PER_DAY && curr_time < (SEEDING_START_DATE+SEEDING_DURATION)*SIM_STEPS_PER_DAY){
        const time_since_seeding_start = curr_time - SEEDING_START_DATE * SIM_STEPS_PER_DAY;
        const seed_doubling_time = SEEDING_DOUBLING_TIME * SIM_STEPS_PER_DAY;
        const current_seeding_rate = SEEDING_RATE_SCALE * Math.pow(2, time_since_seeding_start / seed_doubling_time);
        const num_seeds_curr_time = d3.randomPoisson(current_seeding_rate)();
        let num_seeded = 0;
...
function kappa_T(node, cur_time){
    ...
    if(node["infective"]!==1){
        ...
        const time_since_infection = cur_time - node["time_of_infection"];
...
function psi_T(node, cur_time){
    ...
    const PSI_THRESHOLD = SIM_STEPS_PER_DAY;
    const time_since_infection = cur_time - node["time_of_infection"];
...
function f_kernel(d){
    const a = 10.751; //4. in kms
    const b = 5.384; //3.8. both values are for Thailand, until we get a fit for India
...
function compute_scale_workplaces(workplaces){
    let beta_workplace;
    for (let w=0; w < workplaces.length; w++) {
...
function compute_scale_communities(nodes, communities){
    ...
    for (let w=0; w < communities.length; w++) {

        let sum_value = 0;
        for (let i=0; i<communities[w]['individuals'].length; i++){
...
function compute_scale_public_transport(nodes, public_transports){
    ...
    for (let tr=0; tr < public_transports.length; tr++) {

        let sum_value = 0;
        for (let i=0; i<public_transports[tr]['individuals'].length; i++){
...
function init_homes(){
    ...
    const houses_json = JSON.parse(loadJSON_001('input_files/houses.json'));
    // console.log("In init homes:",houses_json.length,houses_json[0]);
    NUM_HOMES = houses_json.length;

    const homes = [];
    for (let h=0; h < NUM_HOMES; h++) {
        const home = {
...
function init_workplaces(){
    const workplaces_json = JSON.parse(loadJSON_001('input_files/workplaces.json'));
    const schools_json = JSON.parse(loadJSON_001('input_files/schools.json'));
    ...
    for (let w=0; w < NUM_SCHOOLS; w++) {
        ...
        workplaces.push(school)
    }

    for (let w=0; w < NUM_WORKPLACES; w++) {
        let workplace = {
...
function init_community(){
    ...
    const communities_json = JSON.parse(loadJSON_001('input_files/commonArea.json'));
...
function init_public_transport(){
    ...
    const public_transport_entities = [];
    for (let pt=0; pt < NUM_PUBLIC_TRANSPORT; pt++) {
        const public_transport_entity = {
            'loc': [0, 0], // [lat, long]
            'lambda_PT': 0,
            'individuals': [], // We will populate this later
            'Q_c': 1,
...
function euclidean(loc1,loc2) {
    ...
    if ((lat1 === lat2) && (lon1 === lon2)) {
        ...
        let dist = 1 - (Math.sin(radlat1) * Math.sin(radlat2) + Math.cos(radlat1) * Math.cos(radlat2) * Math.cos(radtheta));
        if (dist < 0) {
            dist = 0;
        }
...
function compute_community_distances(communities){
    const inter_ward_distances_json = JSON.parse(loadJSON_001('input_files/wardCentreDistance.json'));

    const community_dist_matrix = math.zeros([communities.length, communities.length]);
    ...
    */
    ...
    for (let c1 =0; c1< inter_ward_distances_json.length; c1++){
        for (let c2=c1+1; c2<inter_ward_distances_json.length; c2++){
            /// console.log(communities[c1]['loc'],communities[c2]['loc'])
...
function get_init_stats(nodes,homes,workplaces,communities){
    for (var h = 0; h< homes.length;h++){
        let working_count  = 0;
        for (let ind_count = 0; ind_count <  homes[h]['individuals'].length; ind_count++){
...
function update_lambda_h_old(nodes, home){
    ...
    for (let i=0; i<home['individuals'].length; i++){
...
function get_age_dependent_mixing_matrix_household(){
    ...
    const sigma_json = JSON.parse(loadJSON_001('Sigma_household.json'));
    const U_matrix_json = JSON.parse(loadJSON_001('U_household.json'));
    const V_matrix_json = JSON.parse(loadJSON_001('Vtranspose_household.json'));
    ...
    for (var count = 0; count < NUM_AGE_GROUPS; count++){
        for (let count2 = 0; count2 < NUM_AGE_GROUPS; count2++){
...
function update_lambda_h(nodes,home){
    ...
    const lambda_age_group = math.zeros([NUM_AGE_GROUPS]);

    if(!USE_AGE_DEPENDENT_MIXING){
        //////////////////////////
        //Sanity test --- use old lambdas
        const lambda_old = update_lambda_h_old(nodes, home);
        ...
        const SIGMA = SIGMA_H;
        const V_MATRIX = V_MATRIX_H;
        const U_MATRIX = U_MATRIX_H;
        const SIGNIFICANT_EIGEN_VALUES = SIGNIFICANT_EIGEN_VALUES_H;

        //add contributions to each age group
        const age_components = math.zeros([NUM_AGE_GROUPS]);
        for (let indv_count = 0; indv_count < home['individuals'].length; indv_count++){
            const indv_age_group = nodes[home['individuals'][indv_count]]['age_group'];
        ...
        const V_T_x = math.zeros([SIGNIFICANT_EIGEN_VALUES]);
...
function update_lambda_w_old(nodes, workplace){
    ...
    for (let i=0; i<workplace['individuals'].length; i++){
        //	var temp = nodes.filter( function(node) {
...
function update_lambda_w(nodes,workplace){
    ...
    if(!USE_AGE_DEPENDENT_MIXING){
        //////////////////////////
        //Sanity test --- use old lambdas
        const lambda_old = update_lambda_w_old(nodes, workplace);
        ...
        let SIGMA = SIGMA_OFFICE;
        let V_MATRIX = V_MATRIX_OFFICE;
        let U_MATRIX = U_MATRIX_OFFICE;
        let SIGNIFICANT_EIGEN_VALUES = SIGNIFICANT_EIGEN_VALUES_OFFICE;
        ...
        //add contributions to each age group
        const age_components = math.zeros([NUM_AGE_GROUPS]);
        for (let student_count = 0; student_count < workplace['individuals'].length; student_count++){
            const student_age_group = nodes[workplace['individuals'][student_count]]['age_group'];
...
function update_lambda_public_transport(nodes, public_transport){
    let sum_value = 0;
    for (let i=0; i<public_transport['individuals'].length; i++){
        //	var temp = nodes.filter( function(node) {
...
function update_lambda_c_local(nodes, community){
    let sum_value = 0;
    for (let i=0; i<community['individuals'].length; i++){
        //	var temp = nodes.filter( function(node) {
        //		return node['index']==community['individuals'][i];
        //	});
...
function update_lambda_c_global(communities,community_distance_matrix){
    for (let c1=0; c1<communities.length; c1++){
        let temp = 0;
        let temp2 = 0;
        for (let c2 = 0; c2<communities.length; c2++){
...
function get_infected_community(nodes, community){
    ...
    let infected_stat = 0;
    var exposed_stat = 0;
    let hospitalised_stat = 0;
    let critical_stat = 0;
    let dead_stat = 0;
    let recovered_stat = 0;
    var exposed_stat = 0;
...
function update_lambda_stats(node){
    const sum_inv = 1 / math.sum(node['lambda_incoming']);
    const lambda_vector = [];
    for (let i= 0; i<node['lambda_incoming'].length; i++){
...
function runday(time_step,homes,workplaces,communities,public_transports,nodes,community_distance_matrix,seed_array,
    ...
    for (let w=0; w<NUM_SCHOOLS+NUM_WORKPLACES; w++){
    ...
    for (let c=0; c<NUM_COMMUNITIES; c++){
        ...
        const temp_stats = get_infected_community(nodes, communities[c]);
    ...
    const n_infected_wardwise = nodes.reduce(function (partial_sum, node) {
    ...
    const n_exposed = nodes.reduce(function (partial_sum, node) {
        return partial_sum + ((node['infection_status'] == EXPOSED) ? 1 : 0);
    }, 0);
    days_num_exposed.push([time_step/SIM_STEPS_PER_DAY, n_exposed]);

    const n_hospitalised = nodes.reduce(function (partial_sum, node) {
    ...
    data_tuple = [days_num_infected,days_num_exposed,days_num_hospitalised,days_num_critical,days_num_fatalities,days_num_recovered,days_num_affected,lambda_evolution];
    console.log("data_tuple=",data_tuple);

    if(time_step>5){
        call_plotly(data_tuple)

        // call_plotly(data_tuple).then(runday(time_step+1,homes,workplaces,communities,public_transports,nodes,community_distance_matrix,seed_array,
        // 	days_num_affected,days_num_critical,days_num_exposed,days_num_fatalities,days_num_hospitalised,days_num_infected,days_num_recovered,lambda_evolution));
        console.log("plotting done");
    } else {
        runday(time_step+1,homes,workplaces,communities,public_transports,nodes,community_distance_matrix,seed_array,
            days_num_affected,days_num_critical,days_num_exposed,days_num_fatalities,days_num_hospitalised,days_num_infected,days_num_recovered,lambda_evolution);
    }
...
function run_simulation() {
    ...
    const days_num_affected = [];
    const days_num_critical = [];
    const days_num_exposed = [];
    const days_num_fatalities = [];
    const days_num_hospitalised = [];
    const days_num_infected = [];
    const days_num_recovered = [];
    const plotly_test_plot = [];
    const infected_count = 0;
    const lambda_evolution = [];
    ...
    NUM_AFFECTED_COUNT = 0;
    const time_step = 0;
    console.log("Got till runday");
    console.log("nodes",nodes);
    runday(time_step,homes,workplaces,communities,public_transports,nodes,community_distance_matrix,seed_array,
        days_num_affected,days_num_critical,days_num_exposed,days_num_fatalities,days_num_hospitalised,days_num_infected,days_num_recovered,lambda_evolution);




    // for(; time_step < NUM_TIMESTEPS; time_step++) {
    // 	console.log(time_step/SIM_STEPS_PER_DAY);
    //
    // 	//seeding strategies
    // 	if(SEEDING_MODE == SEED_INFECTION_RATES && time_step < seed_array.length){
...
function plot_lambda_evolution(data,plot_position,title_text,legends) {
    ...
    for (let count = 0; count < data.length; count++){
        for (let lambda_length_count = 0; lambda_length_count < data[0][0][1].length; lambda_length_count++){
            ...
            for (let count2 = 0; count2 < data[count].length; count2++){
...
function call_plotly(data_tuple) {
    let plot_values = data_tuple;

    initialize_plotly_plot([plot_values[6]],'num_affected_plot_2','Number Affected (cum.)','Evolution of Affected Population');
    initialize_plotly_plot([plot_values[0]],'num_infected_plot_2','Number Infectious (daily)','Evolution of Infected Population');
    //plot_plotly([plot_values[1]],'num_exposed_plot_2','Number Exposed (daily)','Evolution of Exposed Population');
    initialize_plotly_plot([plot_values[2]],'num_hospitalised_plot_2','Number Hospitalised (daily)','Evolution of Hospitalised Population');
    initialize_plotly_plot([plot_values[3]],'num_critical_plot_2','Number Critical (daily)','Evolution of Critical Population');
    initialize_plotly_plot([plot_values[4]],'num_fatalities_plot_2','Number Fatalities (cum.)','Evolution of Fatalities Population');
    ...
    //
    // 	Plotly.extendTraces('num_affected_plot_2', {
    // 		y: [[data_tuple[6]]]
    // 		trace1.x.push(data[count][count2][0]);
    ...
    // )
    //
    // 	if (++cnt >= NUM_TIMESTEPS) clearInterval(interval);
    // }, 300);
...
function initialize_plotly_plot(data,plot_position,title_text,legends){
